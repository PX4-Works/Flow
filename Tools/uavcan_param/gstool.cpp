...
int main(int argc, const char** argv)
{
    if (argc < 3)
    {
        std::printf("Usage: %s <self-node-id> <remote-node-id> [S] save changes\n", argv[0]);
        return 1;
    }
 ...
 *                 */
       ...
       {
           if (p.max_value.is(uavcan::protocol::param::NumericValue::Tag::empty) && p.max_value.is(uavcan::protocol::param::NumericValue::Tag::empty))
           {
           ...
           if (res.first < 0)
           {
               std::printf("Failed to set param: %d\n", res.first);
               std::exit(1);
           }
           std::cout << "Response:\n" << res.second << std::endl;
       }
...
/*
 * If the useer entered S we will save the cureent settting ans exit */
  if (argc > 3 && argv[3][0] == 'S') {
    ...
    if (save_res.first < 0)
    {
        std::printf("Failed to save params: %d\n", save_res.first);
        std::exit(1);
    }

		std::cout << "Param Save response:\n" << save_res.second << std::endl;
    std::cout << "Param Save done!\n\n" << std::endl;
		std::exit(1);
	}
 ...
 */
    ...
    {
        std::printf("Failed to reset params: %d\n", erase_res.first);
        std::exit(1);
    }
    std::cout << "Param reset response:\n" << erase_res.second << std::endl;
    std::cout << "Param reset done!\n\n" << std::endl;
  ...
  */
    ...
    {
        std::printf("Failed to restart: %d\n", restart_res.first);
        std::exit(1);
    }
    std::cout << "Restart response:\n" << restart_res.second << std::endl;
